...
> 実装済み。2026-04-23 に PR #31 でマージ、本番 Supabase に migration 適用済み。  
> evergreen な仕様は [`../../SPEC.md`](../../SPEC.md) を参照。

Sprint A の定量化 + 帯別重み付けで「精度の底」は上がった。Sprint B はその上で **「起草と採点に構造的な反証と追加観点を入れる」** フェーズ。LLM コストがやや増える (Top 10 への追加呼び出し) ため、効果を見ながら段階的に入れた。
