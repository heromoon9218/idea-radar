...
Stack Exchange も同じパターンを踏襲する: `stackexchange.ts` が `metadata.se_site` (14 サイト: parenting / money / workplace / cooking / diy / travel / pets / gardening / fitness / law / outdoors / expatriates / freelancing / pm) を格納 → `analyze.ts:toHaikuInputs` が `se_site` にリフト → `HAIKU_SYSTEM` プロンプトが「育児 / 家計 / 職場 / 料理 / フリーランス実務 / PM」等の痛みとしてクラスタリングの判断材料にする。サイト一覧の単一の真実は `src/collectors/stackexchange.ts:SITES`。

2026-04-29 入替: 雑談寄り・支払意欲弱の lifehacks / interpersonal / academia を削除し、支払文化が強い freelancing / pm に差し替え (productivity SE は SE Network 廃止済みのため不採用、差し引き 15→14 サイト)。詳細経緯は `src/collectors/stackexchange.ts` 冒頭コメント参照。
...
Stack Exchange を主要ソースに据えた (SE 15 サイト × sort=month/hot 2 クエリ) ため、技術系バイアス低減のため HN normal と zenn count を縮小した。これにより日次の収集件数は hatena (~38) + zenn (~30) + HN 非 normal (~75) + HN normal top 30 + stackexchange 15 site (~80-150) = **定常 250-350 件**、初回 ingest 時は SE の month/hot 2 クエリ合計で 700-800 件まで伸びる想定。analyze 側の `MAX_SIGNALS_PER_BATCH` は 1200、`HAIKU_MAX_SIGNALS` は 1000 (週次 wed-fri chunk 3 日窓で ~750-1050 件想定のため 700→1000)。`HAIKU_CONTENT_MAX_CHARS` は 600 (1000 signals × ~150 tok = 155k tok で 200k 上限内に収めるため 800→600 同時引き下げ)。閾値を触る場合はこの収支を確認すること。
