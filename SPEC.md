...
- **非エンジニアまたは B2B 小口が金を払う可能性があるか** — 「開発者向け無料 DevTool」では ¥50k/月 に届きにくいため、需要検証済み・課金文化あり・競合ニッチ な領域を優先する
...
- **足切り条件の入替 (2026-04)**: 旧 `tech_score < 3` 足切りを撤廃し、`market_score >= 3 AND competition_score >= 3 AND distribution リスク high が無いこと` に変更。技術難度では落とさず、市場性・競合性・流通設計の個人開発適合性で落とす方針 (詳細は「分析層 → パイプライン 5)」)
...
- **成功指標（段階的）**:
  ...
  - **長期 (〜12 ヶ月)**: このパイプライン起点のプロジェクト 1 本が **月 ¥50k の収益** に到達する
...
3. 3 役割の候補アイデアを合流 → 役割間 dedup → `raw_score` DESC でソート（Top N 絞り込みは行わない）
4. dedup 後の全候補を既存の Sonnet スコアリング（3 軸）に渡し、Tavily Search で類似サービス検索 → `competitors` に格納
   - **Top N で絞らない設計**: 足切り後に Top 5 を取る構造なので分母が小さいと配信 0 件になりやすいため、足切りより前で件数を絞らない
