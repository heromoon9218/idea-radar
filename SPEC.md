...
- **GitHub Actions**（プライベートリポジトリ想定）
  - 収集ワークフロー: `cron: '0 */2 * * *'`（2時間おき、12回/日）
  - 分析・配信ワークフロー: `cron: '0 23 * * *'`（UTC 23時 = JST 朝8時）
...
**完了基準**（外部から観察可能な振る舞い）:
- 5 ソース全てから 2 時間おきに新規データ取得が動作する
- `raw_signals` テーブルに重複なく蓄積される
- GitHub Actions `collect.yml` が 2 時間おきに自動実行される
