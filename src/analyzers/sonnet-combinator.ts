...
# 痛みドメインの見分け方 (本役割の最重要ミッション)

入力シグナルには 2 系統のソースが混ざっています:
- **非技術ペイン (pain 側の主力)**: Stack Exchange 15 サイト (lifehacks / parenting / money / workplace /
  cooking / diy / interpersonal / travel / pets / gardening / fitness / law / outdoors / expatriates /
  academia)。**既に軽量ドメイン (生活者・家計・職場・趣味) のペイン**なので、そのままでも収益化に載りやすい。
- **技術・情報 (info 側の主力)**: Zenn / はてブ / Hacker News。新 API・ライブラリ・手法・Show HN 等。

pain 側が SE の場合: 既に軽量ドメインのペインなので素直に「info の技術で解く」を書けば良い。
pain 側が技術系 (はてブ / Zenn / HN) しかない場合: 以下の「軽量ドメイン投影」で一般化してから組む。

# 軽量ドメインへの投影 (pain が技術系しかない場合のフォールバック)

pain が技術者の愚痴しかない組み合わせは、素直に組むと「開発者向け DevTool」に偏ります。
しかし開発者コミュニティは OSS 志向・無償前提で支払意欲が低く、
...
- クリエイター・発信者: YouTuber / ストリーマー / VTuber / ブロガー / ポッドキャスター / TikToker
...
- **pain が SE 由来** (se_site が付与されている) の場合: 既に軽量ドメインなので素直にアイデアを 1-2 個組む。WHY には se_site と質問内容の具体引用を含める
- **pain が技術系のみ** の場合: 1 個は軽量ドメインへの投影を必須 (攻めの 1 個)、もう 1 個は素直な技術者向けでも可
- 1 ペアから 1 個だけ出す場合: 収益化に載る方 (SE ペイン直撃 or 軽量ドメイン投影) を優先する
...
# 流通仮説 (distribution_hypothesis) の必須化

「月 5 万円到達は『作れば来る』ではなく『届け方を最初から設計する』」が個人開発の鉄則。
各アイデアに以下 3 フィールドの流通仮説 を必ず付ける:

- channels: 接触候補を 1-3 個 (例: ["業界 Discord サーバー", "Notion Marketplace", "ニッチ系 YouTuber へのアフィリエイト"])
  - 抽象語 ("SNS" / "ブログ" / "口コミ") ではなく、具体名・具体プラットフォーム を書く
  - 軽量ドメインなら、その層が実際にいる場所 (例: Etsy セラー → Etsy フォーラム / Reddit r/EtsySellers)
  - B2B 小口なら、誰のところに直営業するか (例: 「個人税理士事務所 100 件にコールドアウトリーチ」)
- first_10_users: 最初の 10 人を獲得する具体シナリオを 1-3 文
  - 「Twitter で告知」だけは NG。「誰に・何を・どこで・いくらで」まで書く
  - 例: 「個人 EC コンサル X 氏のメルマガ (登録 1,200 人) でレビュー記事 → 早割 50% で 10 件」
- sns_dependency: SNS バイラル依存度 'high' | 'mid' | 'low'
  - high: 「バズ前提」「フォロワー 1 万人前提」「インフルエンサーが取り上げる前提」 — 再現性が低いので減点される
  - mid:  SNS は補助的に使うが、ニッチコミュニティ / 直営業 / SEO もある
  - low:  SNS なしでも届く (B2B 直営業 / 既存ツール連携 / 紹介経由 / 業界出版物)

流通仮説が成立しないアイデア (誰にどう届けるか描けない) は candidates から自主的に除外する (raw_score を下げる)。
...
- **SE ペイン直撃 or 軽量ドメイン投影 が成立しているか (最重要)**: 該当すれば raw_score 4-5、「避けるべき重いドメイン」に投影したものは raw_score 2 以下に抑える、技術者向け単独は最大 3
