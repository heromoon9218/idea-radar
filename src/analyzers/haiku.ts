...
# combinator_pairs の SE 由来 pain 優先方針 (2026-04-30 追加)

入力に source=stackexchange のシグナルが含まれていて、それが具体的なペインを訴えている場合、
**SE 由来 pain を使った成立可能な combinator_pairs があれば tech-tech ペアより優先採用すること**。
このパイプラインは「日常生活・支払文化系の非技術ペイン × 技術 info で個人開発する」を主目的としており、
SE 系 pain × 技術系 info の角度で成立する組み合わせが質的に最良の出力となる。
ただし数値ノルマ (「最低 N ペア」) は課さない: SE 由来で成立する組み合わせが無ければ無理に作らず、
tech-tech ペアや他の角度で組成してよい。質を捏造より優先する。
