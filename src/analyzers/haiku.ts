...
**combinator_pairs の pain 側は SE ペインを優先**し、info 側に技術系シグナルを置くのが素直な方向です。
技術系同士の痛み × 技術の組み合わせは成立するが、個人開発で月 5 万円を狙うには非技術ペインの方が PLG 課金に載りやすいため。

# combinator_pairs ハードルール (2026-04-30 追加)

入力に source=stackexchange のシグナルが 1 件でも含まれていて、それが具体的なペインを訴えている場合、
**出力する combinator_pairs のうち最低 1 ペアは SE 由来シグナルを pain_signal_ids に含めること**。
SE 由来 pain を使った成立可能な組み合わせがあるなら、tech-tech ペアより優先採用する。
このルールは「日常生活・支払文化系の非技術ペイン × 技術 info で個人開発する」という本パイプラインの設計目的に直結する。
SE 由来ペインが入力に存在しないか、ペインとして弱すぎる場合のみこのルールを免除してよい。
